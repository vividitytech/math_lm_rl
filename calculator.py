...
def sample2(model, qn, tokenizer, device, sample_len):
    # Inefficient version of calculator sampling -- no batches, doesn't
    # cache activations from previous tokens
    EQUALS_TOKENS = set([28, 796, 47505])
    ...
    for _ in range(sample_len):
        with th.no_grad():
            ...
            out = model.generate2(
                **toks, max_length=orig_len + 1, greedy=True,output_hidden_states=True, pad_token_id=model.config.eos_token_id
            )
            # handle the multiple outputs
            out = out[0]
            text = tokenizer.batch_decode(out)[0]
...
def greedy_sample(model,qn, tokenizer,device, sample_len):
    # Inefficient version of calculator sampling -- no batches, doesn't
    # cache activations from previous tokens
    EQUALS_TOKENS = set([28, 796, 47505])
    toks = tokenizer([qn], return_tensors="pt").to(device)
    for _ in range(sample_len):
        with th.no_grad():
            orig_len = toks["input_ids"].shape[1]
            ...
            toks["input_ids"]= out
            toks["attention_mask"] = th.ones_like(out)

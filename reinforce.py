...
class PPOTrainer(BaseTrainer):
    ...
    def generate(
        ...
        **generation_kwargs,
        ...
        if isinstance(query_tensor, List):
            ...
            #batch_mask = torch.ones_like(query_tensor)
            batch_mask = [element!=0 for element in query_tensor]
            batch_mask = torch.stack(batch_mask)
            inputs = {"input_ids": query_tensor, "attention_mask": batch_mask.to(int)}
        ...
        **generation_kwargs,
        ...
        for i in range(0, len(query_tensors), batch_size):
            ...
            if gene_type=="default":
                ...
                temp = torch.stack(generations["scores"]).permute(1,0,2)
                temp = [tensor for tensor in temp]
                logits.extend(temp)

                hs = []
                for i in range(0, len(generations['hidden_states'])):
                    hs.append(generations['hidden_states'][i][-1].squeeze(1))
                hs[0] = hs[0][:, -1, :]
                hs = torch.stack(hs).permute(1,0,2)
                hs = [tensor for tensor in hs]
                hidden_states.extend(hs)
            else:
                generations = self.accelerator.unwrap_model(self.model).generate2(**padded_inputs, **generation_kwargs)
                ...
                for i in range(0, len(generations['hidden_states'])):
                    hs.append(generations['hidden_states'][i][-1].squeeze(1))
                hs[0] = torch.unsqueeze(hs[0][-1, :], 0)
                hs = torch.stack(hs).permute(1,0,2)
                hidden_states.extend(hs)
                action_scores.extend(torch.stack(generations["action_scores"]).permute(1,0,2))
        self.tokenizer.padding_side = padding_side_default
        return {"sequences":outputs, "scores":logits, "hidden_states": hidden_states, "action_scores":action_scores}
    ...
    def step(
        ...
        with torch.no_grad():
            all_logprobs, _, values, masks, actions = self.batched_forward_pass(self.model, queries, responses, model_inputs)
            ...
            if self.is_peft_model and hasattr(
                ...
                ref_logprobs, _, _, _, ref_actions = self.batched_forward_pass(self.ref_model, queries, responses, model_inputs)
        ...
        for _ in range(self.config.ppo_epochs):
            ...
            for batch in mini_batch_dataloader:
                with self.accelerator.accumulate(self.model):
                    ...
                    logprobs, logits, vpreds, _, actions = self.batched_forward_pass(
    ...
    def batched_forward_pass(
        ...
        for i in range(int(bs / fbs)):
            ...
            logits, _, values, actions = model(**input_kwargs)
    ...
    def train_minibatch(
        ...
        """
        loss_p, loss_v, train_stats = self.loss2(old_logprobs, values, rewards, logits, vpreds, logprobs, mask)
        #loss = loss_p + loss_v 
        loss =  - torch.mean(logprobs.view(1, -1))
        self.optimizer.zero_grad()
    ...
    def loss2(
        ...
        for t in reversed(range(gen_len)):
            nextvalues = values[:, t + 1] if t < gen_len - 1 else 0.0
            lastgaelam = rewards[:, t] + self.config.gamma * nextvalues
            advantages_reversed.append(lastgaelam)
        advantages = torch.stack(advantages_reversed[::-1]).transpose(0, 1)
